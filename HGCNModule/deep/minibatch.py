...
class EdgeMinibatchIterator(object):
    ...
    """
    def __init__(self, adj_mats, feat, edge_types, batch_size=100, data_set='H_Net', val_test_size=0.01,seed = 0):
    ...
    def mask_test_edges(self, edge_type, type_idx,seed):
        if edge_type==(0,1) or edge_type==(1,0):
            ...
            train_edges = np.loadtxt('./attribute dataset/'+data_set+'/'+ratio+'/train_index_(0, 1)'+str(seed)+'.txt',dtype=int)
            if edge_type == (1, 0):
            ...
            test_edges_false = np.loadtxt('./attribute dataset/'+data_set+'/'+ratio+'/index_test_false(0, 1)'+str(seed)+'.txt',dtype=int)
            if edge_type == (1,0):
            ...
            test_edges = np.loadtxt('./attribute dataset/'+data_set+'/'+ratio+'/test_index_(0, 1)'+str(seed)+'.txt',dtype=int)
            if edge_type == (1, 0):
