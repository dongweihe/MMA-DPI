...
D_MAX = 50
T_MAX = 545
path = '/Users/yang/Desktop/MMA-Master/data/micro attribute/vocabulary/'

#drug_vocab_path = '../vocabulary/drug_bpe_chembl_freq_100.txt'
drug_codes_bpe = codecs.open(path + 'drug_bpe_chembl_freq_100.txt')
drug_bpe = BPE(drug_codes_bpe, merges=-1, separator='')
drug_temp = pd.read_csv(path + 'subword_list_chembl_freq_100.csv')
drug_index2word = drug_temp['index'].values
drug_idx = dict(zip(drug_index2word, range(0, len(drug_index2word))))

target_vocab_path = path + 'target_bpe_uniprot_freq_500.txt'
target_codes_bpe = codecs.open(target_vocab_path)
target_bpe = BPE(target_codes_bpe, merges=-1, separator='')
target_temp = pd.read_csv(path + 'subword_list_uniprot_freq_500.csv')
