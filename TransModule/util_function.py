...
from TransModule.preprocess import drug_encoder, target_encoder
...
def load_davis_dataset():
    ...
    trainn_fold = json.load(
        open(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'folds', 'train_fold_setting1.txt')))
    train_fold = []
    ...
    test_fold = json.load(
        open(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'folds', 'test_fold_setting1.txt')))
    ligands = json.load(
        open(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'ligands_can.txt')),
        object_pairs_hook=OrderedDict)
    proteins = json.load(
        open(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'proteins.txt')),
        object_pairs_hook=OrderedDict)
    
    affinity = pickle.load(open(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'Y'),
                                'rb'), encoding='latin1')
    ...
    os.makedirs(os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'processed'), exist_ok=True)
    train_test_dataset = []
    for split in ['train', 'test']:
        split_dir = os.path.join('intermolecular dataset', 'regression', 'benchmark', 'DAVIStest', 'processed', split)
        os.makedirs(split_dir, exist_ok=True)
        fold = train_fold if split == 'intermolecular dataset' else test_fold
...
def load_DAVIS(convert_to_log=True):
    ...
    Load raw DAVIS
    """
    affinity = pd.read_csv('./intermolecular dataset/regression/DAVIS/affinity.txt', header=None, sep=' ')
    with open('./intermolecular dataset/regression/DAVIS/target_seq.txt') as f1:
        target = json.load(f1)
    with open('./intermolecular dataset/regression/DAVIS/SMILES.txt') as f2:
        drug = json.load(f2)
...
def load_KIBA():
    ...
    Load raw KIBA
    """
    affinity = pd.read_csv('./intermolecular dataset/regression/KIBA/affinity.txt', header=None, sep='\t')
    affinity = affinity.fillna(-1)
    with open('./intermolecular dataset/regression/KIBA/target_seq.txt') as f:
        target = json.load(f)
    with open('./intermolecular dataset/regression/KIBA/SMILES.txt') as f:
        drug = json.load(f)
...
def load_ChEMBL_pkd():
    ...
    """
    affinity = pd.read_csv('./intermolecular dataset/regression/ChEMBL/Chem_Affinity.txt', header=None)
    affinity = affinity.fillna(-1)
    target = pd.read_csv('./intermolecular dataset/regression/ChEMBL/ChEMBL_Target_Sequence.txt', header=None)
    drug = pd.read_csv('./intermolecular dataset/regression/ChEMBL/Chem_SMILES_only.txt', header=None)
...
def load_ChEMBL_kd():
    ...
    """
    affinity = pd.read_csv('./intermolecular dataset/regression/ChEMBL/Chem_Kd_nM.txt', header=None)
    target = pd.read_csv('./intermolecular dataset/regression/ChEMBL/ChEMBL_Target_Sequence.txt', header=None)
    drug = pd.read_csv('./intermolecular dataset/regression/ChEMBL/Chem_SMILES_only.txt', header=None)
...
def load_BindingDB_kd():
    ...
    """
    affinity = pd.read_csv('./intermolecular dataset/regression/BindingDB/BindingDB_Kd.txt', header=None)
    target = pd.read_csv('./intermolecular dataset/regression/BindingDB/BindingDB_Target_Sequence_new.txt', header=None)
    drug = pd.read_csv('./intermolecular dataset/regression/BindingDB/BindingDB_SMILES_new.txt', header=None)
